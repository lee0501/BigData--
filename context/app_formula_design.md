# iMarine Dashboard 指標公式與模型設計

本文件整理 dashboard 所有指標的計算邏輯、公式定義與模型設計依據。  
設計原則以 [bigdata.md](bigdata.md) 為準，目標是讓儀表板從「展示型 prototype」往「公開資料支撐的跨港決策支援工具」靠近。
...
本節整理 Pressure / Buffer 指標的三種權重來源，說明正式權重的選擇邏輯。

### A.1 三種權重來源

本研究使用三種資訊來源作為權重設計依據，不直接宣稱哪一組權重正確：

1. **交通部資料**：說明為何目前的權重邏輯具有政策與營運背景。
2. **ROC 權重**：在僅知道重要性排序時，提供一組方法論上的基準。
3. **自治權重**：依研究目的與解釋需求，自行設定較符合研究敘事的實務權重。

### A.2 交通部資料佐證

來源：國內各國際港埠船舶進出港操作效率比較分析（荷夫曼先生之營運作業績效指標）

一個港口的效率涉及多個因素，其中與本研究最相關者：

- **貨物在港滯留時間**：港口儲存空間最多放 80–85% 滿，否則面臨反覆堆置與貨物損害問題。本研究以當月空櫃淨增加（40% 權重）對應此概念，確立空櫃流動的核心地位。
- **機具使用**：本研究以空櫃是否佔用大量作業資源（35% 權重）對應此指標。
- **近三個月狀況**（25% 權重）：作為輔助資訊，說明港口是否持續在累積。

### A.3 ROC 權重（Ranked Order Centroid）

**文獻來源**：Barron & Barrett (1996), "Decision Quality Using Ranked Attribute Weights", *Management Science*（SSCI Q1，引用超過 1,032 次）

ROC 適用於僅知道各屬性重要性排序、但不確定每一屬性量化比重的情況。3 屬性的 ROC 計算結果：

| 屬性 | 排名 | ROC 權重 |
|------|------|---------|
| `buffer_space_raw`（空間餘裕） | 1 | 0.611 |
| `buffer_net_raw`（歷史淨流出） | 2 | 0.278 |
| `buffer_export_raw`（出口拉力） | 3 | 0.111 |

**ROC 在本研究中的定位**：理論基準情境，不過度依賴研究者主觀加權，提供一組可被文獻支持的比較基準。

### A.4 自治權重（正式採用）

自治權重的選擇邏輯：`pressure_flow_raw` 與 `pressure_net_raw` 的目的相近（都描述空櫃進出狀態），因此給予類似的比重，而非過度強調其中一個。近三個月趨勢作為輔助，給予較低比重。
...
自治權重定位：研究導向情境，明確表達「實際堆積」與「持續累積」在研究問題中的重要性。回測結果顯示自治權重優於 ROC 基準，方可主張其更貼近本研究欲回答的 T+1 問題。
...
3. 以 `t+1` 月的實際結果檢驗高壓港與高緩衝港的合理性
4. 比較 ROC 與自治權重在相同 expanding window 下的回測結果

比較重點不是哪一組「預測最準」，而是哪一組更能支撐 T+1 的港口壓力辨識、更符合研究目標中的可解釋決策支援。

詳細回測執行紀錄見 `weight_backtest_plan.md`。
