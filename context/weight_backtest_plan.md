...
目前專案已新增一條獨立的「官方多年統計基準」支線，用臺灣港務股份有限公司多年統計與 Prophet 建立同港多年空櫃占比基準。該支線不屬於本文件的 Pressure / Buffer 權重回測範圍；它的用途是輔助判斷 iMarine 高壓結果是否接近多年常態高位。
...
既然第一輪比較是 `roc` 優於初始 `custom`，則後續更合理的做法是先在 `roc` 周圍做局部敏感度分析，檢查：
