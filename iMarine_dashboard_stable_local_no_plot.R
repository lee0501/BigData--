    ...
    mainPanel(width = 9,
      tabsetPanel(id = "main_tabs",
        tabPanel("空櫃總覽",
          ...
          div(class = "main-card", h3(class = "card-title", "各港口壓力指標"), uiOutput("pressure_plot"))
        ),
        tabPanel("港口分析",
          br(),
          div(class = "main-card", h3(class = "card-title", "港口月度趨勢"), p(class="card-sub", "顯示所選港口的 pressure_index 與 buffer_index 歷史變化。"), uiOutput("port_trend_plot")),
...
server <- function(input, output, session) {
  ...
  output$pressure_plot <- renderUI({
    df <- month_status()
    if (nrow(df) == 0) return(p("無資料"))
    vals <- pmax(0, pmin(100, round(as.numeric(df$pressure_index) * 100)))
    rows <- lapply(seq_len(nrow(df)), function(i) {
      div(style = "margin:10px 0;",
        div(style = "display:flex;justify-content:space-between;font-weight:700;font-size:13px;",
          span(display_port(df$port[i])),
          span(paste0(vals[i], "%"))
        ),
        div(style = "height:12px;background:#e2e8f0;border-radius:999px;overflow:hidden;margin-top:5px;",
          div(style = paste0("height:12px;width:", vals[i], "%;background:#2563eb;border-radius:999px;"))
        )
      )
    })
    div(rows)
  ...
  output$port_trend_plot <- renderUI({
    selected_port <- normalize_port(input$port)
    df <- status_final[status_final$port == selected_port, , drop = FALSE]
    if (nrow(df) == 0) return(p("無資料"))
    df <- df[order(df$ym), ]
    pres <- pmax(0, pmin(100, round(as.numeric(df$pressure_index) * 100)))
    buff <- pmax(0, pmin(100, round(as.numeric(df$buffer_index) * 100)))
    rows <- lapply(seq_len(nrow(df)), function(i) {
      div(style = "margin:10px 0;padding-bottom:8px;border-bottom:1px solid #e2e8f0;",
        div(style = "font-weight:800;margin-bottom:6px;", fmt_ym(df$ym[i])),
        div(style = "display:flex;align-items:center;gap:8px;margin:4px 0;",
          span(style="width:70px;color:#475569;font-size:12px;", "Pressure"),
          div(style="flex:1;height:10px;background:#e2e8f0;border-radius:999px;overflow:hidden;",
            div(style=paste0("height:10px;width:", pres[i], "%;background:#ef4444;border-radius:999px;"))
          ),
          span(style="width:45px;text-align:right;font-weight:700;", paste0(pres[i], "%"))
        ),
        div(style = "display:flex;align-items:center;gap:8px;margin:4px 0;",
          span(style="width:70px;color:#475569;font-size:12px;", "Buffer"),
          div(style="flex:1;height:10px;background:#e2e8f0;border-radius:999px;overflow:hidden;",
            div(style=paste0("height:10px;width:", buff[i], "%;background:#16a34a;border-radius:999px;"))
          ),
          span(style="width:45px;text-align:right;font-weight:700;", paste0(buff[i], "%"))
        )
      )
    })
    div(rows)
